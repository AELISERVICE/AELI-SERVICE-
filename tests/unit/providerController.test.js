...
jest.mock("../../src/utils/emailTemplates", () => ({
  documentsReceivedEmail: jest
    .fn()
    .mockReturnValue({ subject: "test", html: "test" }),
}));
...
describe("Provider Controller", () => {
  ...
  describe("createProvider", () => {
    ...
    it("should throw error if provider already exists", async () => {
      ...
      await expect(createProvider(mockReq, mockRes, mockNext)).rejects.toThrow(
  ...
  describe("getProviders", () => {
    ...
    it("should filter providers by price range", async () => {
      ...
      const lastCallArgs =
        Provider.findAndCountAll.mock.calls[
          Provider.findAndCountAll.mock.calls.length - 1
        ][0];
  ...
  describe("getProviderById", () => {
    ...
    it("should throw error if provider not found", async () => {
      ...
      await expect(getProviderById(mockReq, mockRes, mockNext)).rejects.toThrow(
        "provider.notFound",
      );
    });
  });
  ...
  describe("updateProvider", () => {
    ...
    it("should handle photo upload and respect limit", async () => {
      ...
      await expect(updateProvider(mockReq, mockRes, mockNext)).rejects.toThrow(
  ...
  describe("getMyDashboard", () => {
    ...
    it("should throw error if user not authenticated", async () => {
      ...
      await expect(getMyDashboard(mockReq, mockRes, mockNext)).rejects.toThrow(
  ...
  describe("Document Management", () => {
    describe("uploadDocuments", () => {
      it("should upload documents successfully", async () => {
        ...
        expect(i18nResponse).toHaveBeenCalled();
        const lastCall =
          i18nResponse.mock.calls[i18nResponse.mock.calls.length - 1];
    ...
    describe("deleteDocument", () => {
      it("should delete a document", async () => {
        ...
        expect(mockProvider.changed).toHaveBeenCalledWith("documents", true);
        expect(mockProvider.save).toHaveBeenCalledWith(
          expect.objectContaining({
            fields: expect.arrayContaining(["documents"]),
          }),
        );
      });
    });
