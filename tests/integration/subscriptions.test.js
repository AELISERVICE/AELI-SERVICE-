...
describe("Subscription Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    testProvider = await Provider.create({
      ...
      description:
        "A test business for subscription testing. This description needs to be at least 50 characters.",
      location: "Douala",
      isVerified: true,
    });
  ...
  describe("Subscription Model Methods", () => {
    ...
    describe("Subscription.renewSubscription()", () => {
      test("should renew subscription with monthly plan", async () => {
        ...
        const renewed = await Subscription.renewSubscription(
          testProvider.id,
          "monthly",
          validPaymentId,
        );
        ...
        const daysRemaining = Math.round(
          (renewed.endDate - new Date()) / (1000 * 60 * 60 * 24),
        );
