...
const data = [
    { id: 1, date: '02/02/2026', time: '10:46:10', eventtype: 'Connexion échoué', ipadresse: '192.168.1.100', user: 'john@creative-tim.com', risklevel: 'Medium', status: 'Echoué' },
            ...
            <Table headers={headers}>
                {data.map((item) => (
                    <tr key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-slate-600">
                            <div className="flex items-center gap-2">{item.risklevel}</div>
                        </td>
                        <td className="px-6 py-4">
                            <Badge
                                status={item.status}
